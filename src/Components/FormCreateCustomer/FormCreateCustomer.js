...
import { TextField, Checkbox, Button, Box, Typography, Divider, Tooltip } from '@mui/material';
...
const FormCreateCustomer = () => {
  ...
  return (
    ...
    <form style={{display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'center', marginTop: '20px'}} onSubmit={handleSubmit(onSubmit)}>        
          <Box sx={{width: {xs: '100%', sm: '100%', md: '48%' }, display: 'flex', flexDirection: 'column', gap: '15px'}}>
            ...
            <TextField 
              ...
              label="Telefone" 
              onChange={(e) => {
                const phone = formatToPhone(e.target.value);
            ...
            <TextField 
              ...
              required 
              onChange={(e) => {
                const cpf = formatToCPFOrCNPJ(e.target.value);
                setFormattedCpf(cpf);
                setValue("cpfCnpj", cpf);
              }}
            />
          ...
          <Box sx={{width: {xs: '100%', sm: '100%', md: '50%' }, display: 'flex', flexDirection: 'column', gap: '15px'}}>
            ...
            <Box sx={{display: 'flex', alignItems: 'center'}}><label>Desativar notificações: <Checkbox {...register("notificationDisabled")} /></label><Tooltip title="Notificações enviadas por e-mail e SMS ao custo de R$0,99, se desabilitadas, o boleto deverá ser enviado por e-mail próprio ou correio"><HelpIcon sx={{cursor: 'pointer'}}/></Tooltip></Box>    
      </Box>
