...
const DoacoesSite = () => {
        ...
        const fetchData = async () => {
            try {
                const requests = Object.keys(externalReferences).map(ref =>
                    axios.post(`${process.env.REACT_APP_URL}/listar-doacoes.php`, { externalReference: ref })
                );
    ...
    return (
        <Box>
            <Typography variant="h4">Doações Site Amigos da Casa</Typography>
                ...
                Object.keys(groupedData).map((projectKey, index) => (
                    <Box key={index} mb={4}>
                        ...
                        {Object.keys(groupedData[projectKey]).map((status, idx) => (
                            <Card key={idx} variant="outlined" sx={{ mb: 2, backgroundColor: statusColors[status] }}>
                                <CardContent>
                                    <Typography variant="h6">Status: {statusTranslations[status]}</Typography>
                                    <Typography variant="body1">
                                        Total de Doações: {groupedData[projectKey][status].count}
                                    </Typography>
                                    <Typography variant="body1">
                                        Soma dos Valores: R$ {groupedData[projectKey][status].totalValue.toFixed(2)}
                                    </Typography>
                                </CardContent>
                            </Card>
                        ))}
